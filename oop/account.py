...
# Transfer Funds: Method to transfer funds from one account to an instance of another account.
    def transfer_funds(self,amount,transfer_account):
        ...
        if self.get_balance() - amount < self.min_balance:
            ...
            self.transaction.append(Transaction(f"You have transferred {transfer_account.account_number}",-amount,"DEBIT"))
            self.__balance-=amount
        return f"Dear {self.owner} the amount you've transferred is {amount} and your balance is {self.get_balance()}"
...
# Request Loan: Method to request a loan amount.
    ...
    def request_loan(self,amount):
        ...
        if loan_amount > loan_limit:
            ...
            self.loan += amount
            self.transaction.append(Transaction("Loan granted",amount,"CREDIT"))
            self.__balance+=amount
...
# Repay Loan: Method to repay a loan with a given amount.
    def repay_loan(self,amount):
        ...
        return f"Dear {self.name} you have repaid {amount} and your loan debt is {self.loan}"

# View Account Details: Method to display the account owner's details and current balance.
    def account_details(self):
        return f"{self.owner} of account{self.__account_number} your account balance is {self.get_balance()} and your loan is {self.loan}"

# Change Account Owner: Method to update the account owner's name.
    def change_details(self,newOwner):
        self.owner = newOwner
        return f"The new owner of the account is {self.owner}"
...
# Interest Calculation: Method to calculate and apply an interest to the balance. Use 5% interest. 
    def calc_interest(self):
        ...
        self.transaction.append(Transaction("Interest applied", interest, "CREDIT"))
